...
class LocalExperiment:
    ...
    def __init__(self, config=None, renderer='headless', erp_amplitude=8.0,
                 noise_amplitude=10.0, verbose=True):
    ...
    def start(self):
        ...
        self.processor = SignalProcessor(proc_client, self.config, verbose=self.verbose)
        return self

    def stop(self):
        for component in (self.simulator,):
    ...
    def calibrate(self, letters=None):
        ...
        worker = threading.Thread(
            target=self.stimulus.run_calibration, args=(letters,),
...
def run_demo(config=None, erp_amplitude=8.0, noise_amplitude=10.0,
             renderer='headless', verbose=True):
    ...
    with LocalExperiment(config, renderer=renderer, erp_amplitude=erp_amplitude,
                         noise_amplitude=noise_amplitude, verbose=verbose) as exp:
        if verbose:
